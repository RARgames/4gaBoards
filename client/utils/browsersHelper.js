...
function checkBrowsers(dir, isInteractive, retry = true) {
  ...
  if (!retry) {
    return Promise.reject(
      new Error(`${chalk.red('As of react-scripts >=2 you must specify targeted browsers.') + os.EOL}Please add a ${chalk.underline('browserslist')} key to your ${chalk.bold('package.json')}.`),
    );
  }
