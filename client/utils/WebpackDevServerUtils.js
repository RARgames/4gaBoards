...
const fs = require('fs');
const isRoot = require('is-root').default;
...
function printInstructions(appName, urls) {
  ...
  console.log();
  console.log(`The development build is not optimized - use ${chalk.cyan('pnpm client:build')} to create a production build.`);
  console.log();
}
  ...
  return detect(defaultPort, host).then(
    (port) =>
      ...
      new Promise((resolve) => {
        ...
        const message =
          process.platform !== 'win32' && defaultPort < 1024 && !isRoot() ? `Admin permissions are required to run a server on a port below 1024.` : `Something is already running on port ${defaultPort}.`;
        if (isInteractive) {
          clearConsole();
