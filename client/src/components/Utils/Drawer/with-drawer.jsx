...
export default (WrappedComponent, defaultProps) => {
  const Drawer = React.forwardRef(({ children, disabled, className, hideCloseButton, closeButtonClassName, wrapperClassName, onClose, ...props }, ref) => {
    ...
    const drawerContent = (
      <FloatingPortal>
        <FloatingOverlay lockScroll className={s.drawerOverlay}>
          <FloatingFocusManager context={context} modal={false} returnFocus={false}>
            {/* eslint-disable-next-line react/jsx-props-no-spreading */}
            <div className={clsx(s.drawer, className, defaultProps?.className)} ref={refs.setFloating} {...getFloatingProps()}>
