...
const Timeline = React.memo(
  ({ rows, zoom, viewDate, markerDate, dependencies, showDependencies, unscheduledBars, emptyText, selectedCardId, onBarClick, onBarReschedule, onBarReassign, onScheduleFromTray, onMarkerSet }) => {
    ...
    const [collapsedBacklogGroups, setCollapsedBacklogGroups] = useState(() => new Set());
    ...
    // Groups the unscheduled tray by board, applying the search query and a render cap so a
    // project with thousands of unscheduled cards doesn't choke the DOM.
    const { backlogGroups, backlogRemaining } = useMemo(() => {
      ...
      const capped = filtered.slice(0, TRAY_LIMIT);
      ...
      return { backlogGroups: order.map((boardId) => byBoard.get(boardId)), backlogRemaining: Math.max(0, filtered.length - TRAY_LIMIT) };
    }, [unscheduledBars, backlogQuery]);

    const toggleBacklogGroup = useCallback((boardId) => {
      setCollapsedBacklogGroups((prev) => {
    ...
    // First visible geometry per cardId, for dependency arrows.
    const barPositionByCardId = useMemo(() => {
      const map = new Map();
      layout.laidOut.forEach(({ row, top, height }) => {
        if (row.isGroup) {
          return;
        }
        (row.bars || []).forEach((bar) => {
          const geometry = resolveBarGeometry(bar);
          if (!geometry || map.has(bar.cardId)) {
            return;
          }
          const startIndex = dayIndexOf(geometry.startDay, rangeStart);
          const endIndex = dayIndexOf(geometry.endDay, rangeStart);
          map.set(bar.cardId, {
    ...
    const dependencyPaths = useMemo(() => {
      ...
      return dependencies
        .map((dep, i) => {
          const from = barPositionByCardId.get(dep.fromCardId);
          const to = barPositionByCardId.get(dep.toCardId);
          if (!from || !to) {
            return null;
          }
          ...
          return { key: `${dep.fromCardId}-${dep.toCardId}-${i}`, d: `M ${x1} ${y1} L ${midX} ${y1} L ${midX} ${y2} L ${x2} ${y2}` };
        })
        .filter(Boolean);
    }, [showDependencies, dependencies, barPositionByCardId]);
              ...
              <>
                ...
                <div className={s.drawerList}>
                  {backlogGroups.map((group) => {
                    const groupCollapsed = collapsedBacklogGroups.has(group.boardId);
                    return (
                  ...
                  {backlogRemaining > 0 && <div className={s.trayMore}>{t('common.andMore', { count: backlogRemaining })}</div>}
                </div>
              </>
            )}
          </div>
              ...
              <div className={s.body} style={{ height: layout.bodyHeight }} onClick={handleGridClick}>
                <div className={s.overlay} style={{ left: LABEL_WIDTH, width: timeWidth, height: layout.bodyHeight }}>
                  ...
                  {dependencyPaths.length > 0 && (
                    <svg className={s.dependencyLayer} width={timeWidth} height={layout.bodyHeight}>
                      {dependencyPaths.map((path) => (
                        <path key={path.key} d={path.d} className={s.dependencyPath} />
                      ))}
                    </svg>
                  )}
                </div>
...
Timeline.propTypes = {
  ...
  dependencies: PropTypes.array, // eslint-disable-line react/forbid-prop-types
